    ...
    .then(data => {
      ...
      function filter() {
        ...
        return filteredData.filter(attack => {
          return parseInt(attack.timestamp) >= epochStart && parseInt(attack.timestamp) <= epochEnd;
        });
      }
      ...
      document.querySelector("#controls .search-date").onchange = filterAndUpdateTotals;
      document.querySelector("#controls .search-date-start").onchange = filterAndUpdateTotals;
      document.querySelector("#controls .search-date-end").onchange = filterAndUpdateTotals;
